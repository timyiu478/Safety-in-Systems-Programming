    ...
    for i in 0..m {
        for j in 0..n {
            if seq1.get(i).unwrap() == seq2.get(j).unwrap() {
                ...
                let _ = grid.set(i+1, j+1, cmp::max(v1, v2));
            }
        }
    }
...
fn main() {
    ...
    let lines1: Vec<String> = read_file_lines(filename1).expect("Invalid filename1");
    let lines2: Vec<String> = read_file_lines(filename2).expect("Invalid filename2");
